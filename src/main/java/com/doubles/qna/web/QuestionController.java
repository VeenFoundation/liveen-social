...
public class QuestionController {
    ...
    // 질문 작성 처리
    @PostMapping
    public String create(String title, String contents, String hashtag, HttpSession session) {
        // 로그인되어 있지 않으면 로그인 페이지로
        if ( !HttpSessionUtils.isLoginUser(session) ) {
            return "redirect:/users/loginForm";
        }
        ...
        Activity newQuestion = new Activity(sessionUser, title, contents, hashtag);
    ...
    private Result valid(HttpSession session, Activity question) {
        // 로그인 여부 체크
        if ( !HttpSessionUtils.isLoginUser(session) ) {
    ...
    private boolean hasPermission(HttpSession session, Activity question) {
        // 로그인 여부 체크
        if ( !HttpSessionUtils.isLoginUser(session) ) {
    ...
    public String updateForm(@PathVariable Long id, Model model, HttpSession session) {
        // 현재 질문 조회
        Activity question = questionRepository.findOne(id);
